...
/*     //Adjusting canvas size to match the screen and also telling WebGL were to render
	...
	*/
	...
	if (!gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)) {
		console.error('ERROR en compilación de vertex shader!', gl.getShaderInfoLog(vertexShader));
		return;
	}
	gl.compileShader(fragmentShader);
	if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
		console.error('ERROR en compilación de fragment shader!', gl.getShaderInfoLog(fragmentShader));
		return;
	}
    ...
    function updateTriangleVertices() {
		...
		triangleVertices = [
			slider1xValue, slider1yValue, 1.0, 1.0, 0.0,
			slider2xValue, slider2yValue, 0.7, 0.0, 1.0,
			slider3xValue, slider3yValue, 0.1, 1.0, 0.6
		];
